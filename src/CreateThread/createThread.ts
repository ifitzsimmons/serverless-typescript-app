...
import { CreateThreadEvent, LambdaResponse } from '../global';
...
const postThread = async ({
  ...
  User,
  PostedTime,
}: CreateThreadEvent): Promise<void> => {
 ...
 */
module.exports.handler = async (event: CreateThreadEvent): Promise<LambdaResponse> => {
  ...
  try {
    await postThread(event);
  } catch (ex) {
    console.error(JSON.stringify(ex.trace));
    response.error = {
      code: ex.code,
      message: ex.message,
    };
